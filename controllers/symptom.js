...
module.exports = {
  ...
  create: (symptom) => {
    const valid = validate(symptom);
    if (!valid) return Promise.reject(valid);
    return r.table('symptoms').insert(symptom, {returnChanges: true}).run()
    .then(firstChange);
  },
  update: (symptom) => {
    const valid = validate(symptom);
    if (!valid) return Promise.reject(valid);
    return r.table('symptoms').update(symptom, {returnChanges: true}).run()
    .then(firstChange);
  },
  delete: (id) => {
    return r.table('symptoms').get(id).delete({returnChanges: true}).run()
    .then(res => {
      return res;
    })
    .then(firstChange);
